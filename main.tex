...
\edittwo{forced to capture robust features that are critical to accurately
...
number of layers.
\edittwo{Meanwhile, the extracted features are forced} to be global, which
...
networks (CNNs),
\edittwo{make use of a set of small filters and share their weights among
...
\edittwo{for the purpose of separating them accurately}.
}  % editone
...
\deletedtwo{to fit the parameters by minimizing the loss $L$};
...
the emission \citep[e.g.,][]{braun1985}.} % edittwo
...
observations that have a sufficiently low noise level.} % editone
...
structures \citep[e.g.,][]{mellema2013rev,mellema2015,koopmans2015rev}.} % edittwo
...
\edittwo{foreground emission suppression and the EoR signal loss}.
...
\deletedtwo{The use of the whole image cubes as the test set enables us to
create complete images of the reconstructed EoR signal.}
} % editone

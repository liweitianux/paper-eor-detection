...
\definechangesauthor[color=magenta]{two}
\newcommand{\addedtwo}[1]{\added[id=two]{#1}}
\newcommand{\deletedtwo}[1]{ \deleted[id=two]{#1}}
\newcommand{\replacedtwo}[2]{\replaced[id=two]{#1}{#2}}
%\renewcommand{\addedtwo}[1]{#1}
%\renewcommand{\deletedtwo}[1]{}
%\renewcommand{\replacedtwo}[2]{#2}
...
\deletedtwo{which states that `a good representation is the one that can be
obtained robustly from a corrupted input and that will be useful for
recovering the corresponding clean input.'
With this training strategy, we}
artificially corrupt (e.g., by adding noise) the original input $\B{x}$,
feed the corrupted input $\B{x}'$ to the autoencoder, and then train it to
reconstruct the original input by minimizing the loss $L(\B{r}, \,\B{x})$.
During this denoising process, the autoencoder is required to
\edittwo{learn the important features in the original input, i.e., a good
representation}%
\deletedtwo{of the input.
Since the trained autoencoder is able to denoise the corrupted input
$\B{x}'$ to reconstruct the original input $\B{x}$, hence it}
\edittwo{An autoencoder trained with such a strategy}
...
\citep[e.g.,][]{grais2017}
\edittwo{and requires much less training resources such as memory and time.
...
obtained in previous layers, building highly expressive CNNs} that achieve
...
differences between the EoR signal and the foreground emission for the
purpose of reconstructing the EoR signal.
}  % editone
...
\editone{%
\edittwo{%
For the CDAE to be proposed, both the encoder and decoder parts consist of
...
\deletedtwo{To some degree, the proposed CDAE is analogous to a CNN without
the fully connected layers needed for classification.}
...
in all layers) of the CDAE are initialised randomly using the He
uniform initialiser \citep{he2015}.
\edittwo{To train these parameters to obtain an effective CDAE, the}
following three datasets are required%
\deletedtwo{to train and evaluate the CDAE}
\citep[e.g.,][]{ripley1996}:
(1) training set ($S_{\R{tr}}$) to fit the parameters%
\deletedtwo{by minimizing the loss $L$};
(2) validation set ($S_{\R{val}}$) not only to validate
\edittwo{whether the training process goes well (e.g., no over-fitting)}
but also to help determine the hyperparameters (e.g., the number of
layers and filters; \autoref{sec:architecture});
...
one sky pixel\deletedtwo{to be fed into the CDAE}, and $\B{x}_{\R{eor}}$
is the corresponding EoR signal\deletedtwo{in the same pixel}.
...
\edittwo{outputting the reconstructed EoR signal} $\B{r}^{(i)}_{\R{eor}}$.
...
\edittwo{the parameters are updated to reduce the loss $L$, improving}
the quality of the reconstructed EoR signal.
As the training goes for more epochs, the initially randomised CDAE is
gradually shaped into a network
\edittwo{that learns a better representation of the input and can
...
To evaluate the performance of the \edittwo{trained} CDAE%
\deletedtwo{in reconstructing the EoR signal},
...
\end{equation}
where
\edittwo{$\bar{r}_{\R{eor}}$ and $\bar{x}_{\R{eor}}$ represent the mean
...
\addedtwo{$\rho(\B{r}_{\R{eor}}, \B{x}_{\R{eor}})$} is,
...
\edittwo{%
we take one random sky pixel as an example and show in the foreground
spectra with and without the beam effects in \autoref{fig:simudata}, where
the corresponding differential spectra and the EoR signal spectrum are also
plotted.}
...
resulted from the beam effects.} % editone
\edittwo{%
Although such fluctuations exhibit somewhat similar spectral scales
($\lesssim \SI{1}{\MHz}$) as the EoR signal, they are still sufficiently
different, which can be exploited by the CDAE to achieve an effective
separation.
We note that the `observed' foreground has an amplitude of about two orders
of mangitude smaller than the ideal foreground, the major reason for which
is that interferometers are only sensitive to the spatial fluctuations of
the emission.} % edittwo

\begin{figure}
  \centering
  \includegraphics[width=\columnwidth]{simudata}
  \caption{\label{fig:simudata}\edittwo{%
    Example spectra (the bold blue lines) of the foreground emission and
    the EoR signal for one random sky pixel.
    The corresponding differential spectra for the foreground emission are
    also plotted in thin red lines.
    The top and middle panels show the foreground emissions without and
    with beam effects considered, respectively, and the bottom panel
    presents the corresponding EoR signal.
  }}
\end{figure}
...
the root-mean-square (r.m.s\@.) amplitudes of the foreground emission

...
representation.}%
\deletedtwo{Since the trained autoencoder is able to denoise the corrupted
input $\B{x}'$ to reconstruct the original input $\B{x}$, hence it}
...
\edittwo{not only to validate whether the training process goes well
(e.g., no over-fitting) but also to}
...
\begin{figure}
  ...
  \caption{\label{fig:simudata}\edittwo{%
    ...
    with the beam effects considered, respectively, and the bottom panel
